...
export const InvitationContent: React.FC<InvitationContentProps> = ({ greeting, couple }) => {
  return (
    <div className="text-center animate-fade-in-up">
      ...
      <div className="mb-12">
        {greeting.split('\n').map((line, i) => (
          <p key={i} className="text-[17px] leading-[2.2] text-[#3d5568] font-light">
            {line}
          </p>
        ))}
      </div>
      ...
      <div className="space-y-4 text-base text-[#3d5f78]">
        <div className="flex items-center justify-center gap-2">
          <span className="text-[15px] tracking-[0.1em]">{couple.groomFather}</span>
          <span className="text-[13px] text-[#a5c8e4]">·</span>
          <span className="text-[15px] tracking-[0.1em]">{couple.groomMother}</span>
          <span className="text-sm tracking-[0.1em] text-[#6a9bc0] ml-1">의 아들</span>
        ...
        <div className="flex items-center justify-center gap-2">
          ...
          <span className="text-sm tracking-[0.1em] text-[#6a9bc0] ml-1">의 딸</span>
